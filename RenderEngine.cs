...
namespace OpenUtau.Core.Render {
    ...
    class RenderEngine {
        ...
        // จำนวน thread สูงสุดสำหรับ render พร้อมกัน (2 threads เพื่อเสถียรภาพ)
        private static readonly SemaphoreSlim RenderSemaphore = new SemaphoreSlim(2, 2);
        ...
        private void RenderRequests(
            ...
            // Render แบบ parallel ด้วย SemaphoreSlim (max 2 threads พร้อมกัน)
            var renderTasks = tuples.Select(tuple => Task.Run(async () => {
            ...
            try {
                Task.WhenAll(renderTasks).Wait(token);
            } catch (OperationCanceledException) {
                // ยกเลิก render อย่างปลอดภัย
            } catch (AggregateException ae) {
                // กรอง OperationCanceledException ออก ส่งต่อ exception จริง
                var real = ae.Flatten().InnerExceptions
                    .Where(e => e is not OperationCanceledException)
                    .ToList();
                if (real.Count > 0) throw new AggregateException(real);
            }
            progress.Clear();
        }
